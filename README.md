...
1. **Sales pipeline.** Sign in as *Maya K. (Account Executive)* and open **Pipeline**: open deals only, as a **Board** (drag between stages) or a **Table** (sortable, stage menu per row). Each rep's choice is remembered. **My deals / Team** switches scope.
   - **Next best action** on every deal, as one line (a short title and one button, the full explanation on hover or click), with its other signals as icons beside it; Good morning lists the day's top actions across deals. Signals: prospects who've gone quiet (no reply in 7+ days, high priority at 14) with a drafted follow-up that's logged in HubSpot; close dates that have passed; deals stuck in a stage; missing deal info; and **a colleague won a similar deal** (same segment and ERP, similar size, shared modules), with the business value and what worked, plus **Ask in Slack** to DM that colleague.
