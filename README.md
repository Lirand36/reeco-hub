...
1. **Sales pipeline.** Sign in as *Maya K. (Account Executive)* and open **Pipeline**: open deals only, as a **Board** (drag between stages) or a **Table** (sortable, stage menu per row). Each rep's choice is remembered. **My deals / Team** switches scope.
   - **Next best action** on every deal, as one line: status as plain text (a short title plus labels like *Close date* or *Similar win*, explained on hover) and actions as buttons: the main one, plus **⋯** for the deal's other actions, each with its reason; Good morning lists the day's top actions across deals. Signals: prospects who've gone quiet (no reply in 7+ days, high priority at 14) with a drafted follow-up that's logged in HubSpot; close dates that have passed; deals stuck in a stage; missing deal info; and **a colleague won a similar deal** (same segment and ERP, similar size, shared modules), with the business value and what worked, plus **Ask in Slack** to DM that colleague.
...
4. **Customer Success.** Sign in as *Dana S. (Customer Success)*.
   - **My portfolio:** her accounts ranked by risk (high → medium → healthy, then the soonest renewal and the biggest ARR), with ARR at risk, the top reasons, a 12-week usage trend, renewal, open support and feature requests per account.
   - **Health score (0–100), explainable:** usage trend 30%, adoption 15%, support load 20%, platform stability 15%, sentiment 20%. Each part shows its score and the reason; a renewal within 90 days amplifies other warning signs. High risk < 50, medium < 70.
   - **Usage anomalies (Snowflake):** last week vs the 4 weeks before; a 30%+ drop in POs, AI invoices or active users, or an ERP sync-error spike, is an anomaly. **Check for anomalies** (also every 30 minutes automatically) DMs the CSM in Slack; **Mark reviewed** saves a HubSpot note.
   ...
   - **Feature requests:** closing a support conversation as "Feature request" logs it in Jira (`PROD`) or adds the account to a matching existing request. Status changes arrive from Jira (`/webhooks/jira`, or **Simulate Jira update** in the demo) and DM the CSMs of every account that asked. When it ships, **Tell the customer** sends an Intercom message and a HubSpot note.
